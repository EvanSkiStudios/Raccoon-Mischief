...
#====useability=====
execute as @e[type=minecraft:armor_stand,tag=dryingrack,tag=drying_rawhide,nbt={HandItems:[{id:"minecraft:beetroot_seeds",Count:1b,tag:{CustomModelData:827200,display:{Name:'{"text":"Raw Hide","italic":false}'}}},{}]}] at @s unless entity @e[type=area_effect_cloud,tag=dryingrack,tag=drying_rawhide,distance=0..0.5,sort=nearest,limit=1] run function raccoon:custom/blocks/drying_rack/drying/usability/dr_dryingtime_rh_to_le