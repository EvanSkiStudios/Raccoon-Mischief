...
#creates area effect cloud block
execute as @e[type=potion,tag=milk,tag=!broken] as @s at @s run execute unless block ~ ~-0.7 ~ air run summon area_effect_cloud ~ ~ ~ {Particle:"entity_effect",Radius:5f,Duration:100,Color:16777215,Tags:["milk"]}
execute as @e[type=potion,tag=milk] as @s at @s run execute unless block ~ ~-0.7 ~ air run tag @s add broken
